...
## For each of the three days we consider
for(fname in c("intermediates/data_for_dir_plots_Zone11_i10.rda",
    ...
    ## Consider both filtered and forecasted dynamics
    load(fname)
    ...
    ## For each grid cell in coarse grid
    for(i in 1:nrow(sgrid_sub)) {

      ## Filter directions at the centre of this grid cell and translate
      ...
      angles_df_sub <- filter(angles_df, s1 == sgrid_sub[i,1] &
       ...
       ## Plot radial histogram (bins of 30 degrees)
       gi <- ggplot() +
           geom_histogram(data = filter(angles_df_sub,
                                        Group == "Updated"),
                           aes(x = Angle), colour = "black",
