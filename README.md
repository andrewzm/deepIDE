...
This GitHub page provides code for reproducing the results in the manuscript titled *Deep Integro-Difference Equation Models for Spatio-Temporal Forecasting*. The manuscript describes the use of Convolution Neural Networks (CNNs) to learn about the dynamic evolution of environmental processes. Once the CNN trained using analyses data (full, complete, data) of a certain phenomenon, it can be used within statistical models used for analysing or forecasting similar or other environmental phenomena that exhibit similar physical behaviour.  
